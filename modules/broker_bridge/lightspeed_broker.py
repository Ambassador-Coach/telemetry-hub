...
from core.events import (
    ...
    BrokerOrderSentEvent, BrokerOrderSentEventData  # New broker gateway events
)
...
class LightspeedBroker(IBrokerService):
    ...
    def __init__(self,
                 event_bus: IEventBus,
                 config_service: Any,
                 correlation_logger: Optional[ICorrelationLogger] = None,  # BROKER ISOLATION: Make optional
                 order_repository: Optional[IOrderRepository] = None,
                 **kwargs):
        self.logger = logging.getLogger("broker_bridge_logger")
        self.event_bus = event_bus
        self._config = config_service
        self._correlation_logger = correlation_logger  # Can be None during broker isolation test
        self._order_repository = order_repository

        self.client = LightspeedBridgeClient(
            host=self._config.get('LIGHTSPEED_HOST', '127.0.0.1'),
            port=self._config.get('LIGHTSPEED_PORT', 9999)
        )
        ...
        # Optional services (may not be available during initialization)
        self._benchmarker = None  # Performance benchmarker - optional
        self._price_provider = kwargs.get('price_provider', None)  # Price provider - optional
        self._pipeline_validator = None  # Pipeline validator - optional
        
        # Redis/Telemetry publishing queue
        import queue
        self._redis_publish_queue = queue.Queue(maxsize=1000)  # For telemetry publishing
        
        # Order validation tracking
        self._validation_id_lock = threading.Lock()
        self._validation_id_mapping = {}  # Maps local_order_id -> validation_id
        self._local_order_to_validation_id = {}  # Maps local_order_id -> validation_id (alternative mapping)
        self._validation_id_to_local_order = {}  # Reverse mapping: validation_id -> local_order_id
        
        # Correlation tracking
        self._correlation_lock = threading.Lock()
        self._order_correlations = {}  # Maps local_order_id -> correlation_id
        
        # Position update throttling
        self._full_position_event_min_interval_sec = 0.5  # Throttle position updates to 2Hz max
        self._last_full_position_event_time = 0.0  # Track last position event time
        self._position_push_filter_lock = threading.Lock()  # Position push filtering lock
        self._last_published_full_broker_position_state = {}  # Track last published position state per symbol
        self._last_full_position_event_publish_time = {}  # Track last publish time per symbol for throttling
        
        self.logger.info("LightspeedBroker initialized. Connection will be established on start().")

    def establish_connection(self) -> bool:
        """
        PHASE 0: Complete broker initialization including ALL threading and handshake logic.
        This method does EVERYTHING needed to get the broker 100% operational.
        Returns True only if broker is completely ready for trading operations.
        """
        self.logger.critical("PHASE 0 HANDSHAKE: Starting complete broker initialization...")
        ...
        try:
            ...
            # Step 2: TANK MODE initialization
            tank_mode = hasattr(self._config, 'ENABLE_IPC_DATA_DUMP') and not self._config.ENABLE_IPC_DATA_DUMP
            if tank_mode:
                self.logger.info("PHASE 0: TANK_MODE activated - IPC data dumping disabled.")
                self._tank_mode = True
            else:
                self._tank_mode = False

            # Step 3: Create and start listener thread
            self.logger.critical("PHASE 0: Creating listener thread...")
            self._listen_thread = threading.Thread(
                target=self._listen_loop, 
                daemon=True, 
                name="LightspeedListenThread"
            )
            self._listen_thread.start()
            time.sleep(0.2)  # Give thread time to start and be ready
            
            # Step 4: Establish socket connection
            self.logger.critical("PHASE 0: Establishing socket connection...")
            self.client.connect()
            
            # RACE CONDITION FIX: Signal that socket is ready for listener thread
            self._socket_ready_event.set()
            self.logger.critical("PHASE 0: Socket ready event signaled to listener thread")
            time.sleep(0.1)  # Brief pause for socket to be ready
            
            # Step 5: Perform handshake
            self.logger.critical("PHASE 0: Performing handshake...")
            req_id = get_next_req_id()
            fut = concurrent.futures.Future()
            store_pending_request(req_id, fut)
            handshake_msg = {"cmd": "ping", "req_id": req_id}

            if not self.client.send_message_no_read(handshake_msg):
                raise ConnectionError("PHASE 0: Handshake failed - Could not send ping.")
            
            # Step 6: Wait for pong response
            self.logger.critical("PHASE 0: Waiting for pong response...")
            response = fut.result(timeout=10.0)  # Generous timeout for handshake
            
            # Step 7: Validate response (match actual Lightspeed format)
            self.logger.critical(f"PHASE 0: Received response: {response}")
            if (response and 
                response.get("event") == "ping_response" and
                response.get("ok") is True and 
                response.get("reply") == "pong" and 
                response.get("req_id") == req_id):
                
                with self._lock:
                    self._is_fully_connected = True
                    self._handshake_completed = True
                
                # Step 8: Final validation - ensure everything is working
                if self._listen_thread and self._listen_thread.is_alive():
                    self.logger.critical("PHASE 0 HANDSHAKE: COMPLETE SUCCESS - Broker 100% operational!")
                    self.logger.critical(f"PHASE 0: Socket connected: {self.client.sock is not None}")
                    self.logger.critical(f"PHASE 0: Listen thread alive: {self._listen_thread.is_alive()}")
                    self.logger.critical(f"PHASE 0: Handshake completed: {self._handshake_completed}")
                    return True
                else:
                    raise ConnectionError("PHASE 0: Listen thread died during handshake")
            else:
                expected = f'{{"event":"ping_response","ok":true,"reply":"pong","req_id":{req_id}}}'
                raise ConnectionAbortedError(f"PHASE 0: Invalid handshake response - Expected: {expected}, Got: {response}")

        except Exception as e:
            self.logger.critical(f"PHASE 0 HANDSHAKE: FATAL FAILURE - {e}", exc_info=True)
            with self._lock:
                self._is_fully_connected = False
                self._handshake_completed = False
            ...
            try:
                if hasattr(self, 'client') and self.client:
                    self.client.close()
                if hasattr(self, '_listen_thread') and self._listen_thread:
                    self._stop_event.set()
                    self._listen_thread.join(timeout=2.0)
            except Exception as cleanup_error:
                self.logger.error(f"PHASE 0: Error during cleanup: {cleanup_error}")
            
            return False

    def start(self):
        """
        The ILifecycleService start method. In the PHASE 0 design, establish_connection()
        already does everything. This method just validates that broker is ready.
        """
        if not self.is_ready:
            self.logger.error("start() called but broker is not ready. establish_connection() must be called first.")
            raise RuntimeError("Broker not ready - establish_connection() must be called before start()")
        
        self.logger.info("Broker start() called - broker already established and ready from PHASE 0.")
    ...
    def _publish_broker_raw_message_to_redis(self, raw_message_dict: dict, message_type: str = "INBOUND", correlation_id_override: Optional[str] = None):
        """Publish raw broker message to Redis stream via CorrelationLogger."""
        # TANK MODE: Skip telemetry publishing if IPC data dumping is disabled
        if hasattr(self, '_tank_mode') and self._tank_mode:
            self.logger.debug("TANK_MODE: Skipping broker raw message publishing (IPC data dumping disabled)")
            return
            
        if not self._correlation_logger:
            # Don't log error for every message - just debug
            self.logger.debug("Cannot publish broker raw message to Redis: CorrelationLogger not available.")
            return
    ...
    def _publish_broker_error_to_redis(self, error_message: str, error_details: str = "", is_critical: bool = False):
        """Publish broker error to Redis stream via CorrelationLogger."""
        # TANK MODE: Skip telemetry publishing if IPC data dumping is disabled
        if hasattr(self, '_tank_mode') and self._tank_mode:
            self.logger.debug("TANK_MODE: Skipping broker error publishing (IPC data dumping disabled)")
            return
    ...
    def _publish_broker_submission_error_to_redis(self, local_order_id: Optional[str], symbol: str, 
        ...
        # TANK MODE: Skip telemetry publishing if IPC data dumping is disabled
        if hasattr(self, '_tank_mode') and self._tank_mode:
            self.logger.debug("TANK_MODE: Skipping broker submission error publishing (IPC data dumping disabled)")
            return
    ...
    def _perform_connection_and_handshake(self):
        """Runs in a temporary thread to handle the blocking connection and handshake."""
        try:
            time.sleep(0.1)
            if not (self._listen_thread and self._listen_thread.is_alive()):
                raise ConnectionError("Listener thread failed to start, cannot perform handshake.")
            ...
            handshake_msg = {"cmd": "ping", "req_id": req_id}
            if not self.client.send_message_no_read(handshake_msg):
                raise ConnectionError("Handshake failed: Could not send ping.")
            
            response = fut.result(timeout=5.0) 
            
            if response and response.get("reply") == "pong":
                self.logger.info("Handshake successful. Broker is now connected.")
                with self._lock:
                    self._is_fully_connected = True
                    self._handshake_completed = True
            else:
                raise ConnectionAbortedError(f"Handshake failed. Invalid response: {response}")

        except Exception:
            self.logger.critical("Broker connection failed during handshake.", exc_info=True)
            with self._lock:
                self._is_fully_connected = False
            self.client.close()
        
        self.logger.info("Connection/handshake thread finished its work and is now exiting.")
    ...
    def is_connected(self) -> bool:
        return self._is_fully_connected and self.client and self.client.sock is not None

    @property
    def is_ready(self) -> bool:
        """The broker is ready ONLY if the handshake is complete and the listener is alive."""
        with self._lock:
            is_connected = self._is_fully_connected and self._handshake_completed
        
        thread_alive = self._listen_thread and self._listen_thread.is_alive()
        
        if not (is_connected and thread_alive):
            self.logger.warning(f"LSB_READINESS_CHECK: FAILED. IsConnectedFlag={is_connected}, ListenThreadAlive={thread_alive}")

        return is_connected and thread_alive

    def stop(self):
        """Stops all broker-related threads and closes the connection."""
        ...
        # RACE CONDITION FIX: Reset socket ready event for future connection attempts
        self._socket_ready_event.clear()
        
        self.logger.info("LightspeedBroker stopped.")
    ...
    def place_order(self,
        ...
        # Capture T18_BrokerSend_ns milestone timing for precise broker send measurement
        t18_broker_send_ns = time.time_ns()
        send_result = self.client.send_message_no_read(req)
        
        # Create and publish BrokerOrderSentEvent with T18 timing and causation
        broker_sent_data = BrokerOrderSentEventData(
            symbol=symbol,
            local_order_id=str(local_order_id) if local_order_id is not None else None,
            order_side=side.upper(),
            order_quantity=float(order_quantity_int),
            order_type=order_type,
            limit_price=limit_price,
            stop_price=stop_price,
            time_in_force=time_in_force,
            broker_request_id=str(rid),
            broker_payload=req.copy(),  # Copy of exact payload sent to broker
            broker_endpoint="lightspeed_cpp_bridge",
            T18_BrokerSend_ns=t18_broker_send_ns,
            submission_timestamp=time.time(),
            submission_successful=send_result,
            correlation_id=correlation_id,
            source_component="LightspeedBroker",
            error_message=None if send_result else "Failed to send order to broker bridge"
        )
        
        # Publish the broker sent event regardless of success/failure for complete observability
        self.event_bus.publish(BrokerOrderSentEvent(data=broker_sent_data))

        # WORLD-CLASS INSTRUMENTATION: Fire-and-forget T18 milestone telemetry
        if self._correlation_logger:
            try:
                telemetry_payload = {
                    # Milestone timestamp (nanoseconds)
                    "T18_BrokerSend_ns": t18_broker_send_ns,
                    ...
                    "broker_endpoint": "lightspeed_cpp_bridge",
                    "send_successful": send_result,
                    "message_type": req.get("message_type"),
                    
                    # Causation chain
                    "correlation_id": correlation_id,
                    "validation_id": validation_id,
                    "broker_sent_event_id": broker_sent_data.event_id
                }
                
                # Fire and forget - no blocking!
                self._correlation_logger.log_event(
                    source_component="LightspeedBroker",
                    event_name="BrokerSendMilestone",
    ...
    @log_function_call('broker_bridge')
    def cancel_order(self, broker_order_id: Union[str, int], local_order_id: Optional[str] = None) -> None:
        ...
        if send_result:
            ...
            if local_order_id: # Publish pending cancel status
                ...
                status_data = OrderStatusUpdateData(
                    local_order_id=local_order_id,
                    broker_order_id=str(ls_order_id),
                    symbol="",
                    status=OrderLSAgentStatus.PENDING_CANCEL,
                    message=f"Cancel request sent for order ID {ls_order_id}",
                    timestamp=time.time(),
                    correlation_id=correlation_id  # FUZZY'S FIX: Include correlation ID
                )
                self.event_bus.publish(OrderStatusUpdateEvent(data=status_data))
    ...
    def _listen_loop(self) -> None:
        self.logger.critical("LSB_LISTEN_THREAD: Loop started.")
        self.logger.info("Entering Lightspeed listen loop.")
        
        # RACE CONDITION FIX: Wait for socket to be ready before proceeding
        self.logger.critical("LSB_LISTEN_THREAD: Waiting for socket ready signal...")
        if not self._socket_ready_event.wait(timeout=5.0):
            self.logger.critical("LSB_LISTEN_THREAD: TIMEOUT waiting for socket ready signal - proceeding anyway")
        else:
            self.logger.critical("LSB_LISTEN_THREAD: Socket ready signal received - proceeding with message dispatching")
 ...
 # C++ sends rejections within order_status mostly
    ...
    def _handle_order_status_update(self, data: dict, perf_timestamps: Optional[Dict[str, float]] = None) -> None:
        # Capture T19_BrokerMessageReceived_ns milestone timing immediately upon broker message receipt
        t19_broker_received_ns = time.time_ns()
        ...
        try:
            ...
            if actual_local_id_to_use:
                with self._correlation_lock:
                    ...
                    if stored_correlation_id:
                        ...
                        self.logger.debug(f"No correlation found for local_id '{actual_local_id_to_use}'")
            ...
            status_data = OrderStatusUpdateData(
                ...
                ephemeral_corr=ephemeral_corr_str, # <<< ADD THIS LINE
            ...
            # Publish broker message received metadata with T19 timing via telemetry service
            if self._correlation_logger:
                # Handle orphaned events when correlation ID lookup fails
                if not stored_correlation_id:
                    orphaned_event_metadata = {
                        "T19_BrokerMessageReceived_ns": t19_broker_received_ns,
                        "message_type": "ORPHANED_ORDER_STATUS_UPDATE",
                        "broker_status": raw_broker_status_str,
                        "local_order_id": actual_local_id_to_use,
                        "broker_order_id": broker_order_id_str,
                        "symbol": final_symbol_str,
                        "orphan_reason": "CORRELATION_ID_LOOKUP_FAILED",
                        "inbound_message_payload": data.copy()
                    }
                    
                    target_stream = getattr(self._config, 'redis_stream_broker_communication', 'testrade:broker-communication')
                    self._correlation_logger.log_event(
                        source_component="LightspeedBroker",
                        event_name="BrokerOrphanedMessage",
                        payload=orphaned_event_metadata
                    )
                    success = True
                    
                    if success:
                        self.logger.warning(f"Orphaned broker message published with T19 timing for local_order_id={actual_local_id_to_use}")
                    else:
                        self.logger.error(f"Failed to publish orphaned broker message for local_order_id={actual_local_id_to_use}")
                
                elif stored_correlation_id:
                    # Try to retrieve T18 timestamp for latency calculation
                    t18_broker_send_ns = None
                    t18_to_t19_latency_ms = None
                    
                    # Look up the order to get T18 timing if available
                    if self._order_repository and actual_local_id_to_use:
                        try:
                            order_obj = self._order_repository.get_order_by_local_id(actual_local_id_to_use)
                            if order_obj and hasattr(order_obj, 'broker_gateway_metadata'):
                                t18_broker_send_ns = order_obj.broker_gateway_metadata.get('T18_BrokerSend_ns')
                                if t18_broker_send_ns:
                                    t18_to_t19_latency_ms = (t19_broker_received_ns - t18_broker_send_ns) / 1_000_000
                        except Exception as lookup_error:
                            self.logger.debug(f"Could not lookup T18 timing: {lookup_error}")
                    
                    broker_message_metadata = {
                        "T19_BrokerMessageReceived_ns": t19_broker_received_ns,
                        "T18_BrokerSend_ns": t18_broker_send_ns,  # May be None
                        "T18_to_T19_latency_ms": t18_to_t19_latency_ms,  # Broker round-trip time
                    ...
                    self._correlation_logger.log_event(
                        source_component="LightspeedBroker",
                        event_name="BrokerMessageReceived",
                        payload=broker_message_metadata
                    )
                    success = True
                    
                    if success:
                        self.logger.debug(f"Broker message received metadata published with T19 timing for correlation_id={stored_correlation_id}")
                    else:
                        self.logger.warning(f"Failed to publish broker message received metadata for correlation_id={stored_correlation_id}")
            ...
            if standardized_status_enum in [OrderLSAgentStatus.FILLED, OrderLSAgentStatus.PARTIALLY_FILLED] and fill_quantity_val > 0:
                ...
                # Publish broker fill message received metadata with T19 timing via telemetry service
                if self._correlation_logger and stored_correlation_id:
                    broker_fill_metadata = {
                        "T19_BrokerMessageReceived_ns": t19_broker_received_ns,
                        "message_type": "ORDER_FILL",
                        "broker_status": raw_broker_status_str,
                        "local_order_id": actual_local_id_to_use,
                        "broker_order_id": broker_order_id_str,
                        "symbol": final_symbol_str,
                        "fill_quantity": fill_quantity_val,
                        "fill_price": average_fill_price_val,
                        "fill_side": side_enum.value if hasattr(side_enum, 'value') else str(side_enum),
                        "correlation_id": stored_correlation_id,
                        "causation_id": causation_id,
                        "fill_timestamp": final_fill_timestamp,
                        "inbound_message_payload": data.copy()
                    ...
                    self._correlation_logger.log_event(
                        source_component="LightspeedBroker",
                        event_name="BrokerFillReceived",
                        payload=broker_fill_metadata
                    )
                    success = True
                    
                    if success:
                        self.logger.debug(f"Broker fill received metadata published with T19 timing for correlation_id={stored_correlation_id}")
                    else:
                        self.logger.warning(f"Failed to publish broker fill received metadata for correlation_id={stored_correlation_id}")
    ...
    def _handle_account_update(self, data: dict, perf_timestamps: Optional[Dict[str, float]] = None) -> None:
        try:
            ...
            self.event_bus.publish(AccountUpdateEvent(data=account_summary, correlation_id=f"acc_update_{get_thread_safe_uuid()[:8]}"))
    ...
    def _publish_service_health_status(self, health_status: str, status_reason: str, 
        ...
        """
        if not self._correlation_logger:
            self.logger.debug("Cannot publish service health status: CorrelationLogger not available")
            return
        ...
        try:
            ...
            # Publish via correlation logger
            self._correlation_logger.log_event(
                source_component="LightspeedBroker",
            ...
            # Use correlation logger for non-blocking publish
            self._correlation_logger.log_event(
                source_component="LightspeedBroker",
                event_name="ServiceHealthStatus",
                payload=payload
    ...
    def _publish_periodic_health_status(self) -> None:
        ...
        try:
            ...
            context = {
                ...
                "price_provider_available": self._price_provider is not None,
                "order_repository_available": self._order_repository is not None,
                "telemetry_service_available": self._correlation_logger is not None,
