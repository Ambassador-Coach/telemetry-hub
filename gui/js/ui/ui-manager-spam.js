...
export class UIManager {
    ...
    updateROIDisplay() {
        ...
        if (previewImage && roiBox && previewImage.naturalWidth > 0 && currentROI?.length === 4) {
            // Show ROI box for both raw and processed images
            const currentMode = this.state.getImageMode();
            console.log(`🎯 Showing ROI box for ${currentMode} mode`);
            ...
            // Calculate ROI box position and size using actual coordinates
            const roiLeft = x1 * scaleX;
            const roiTop = y1 * scaleY; 
            const roiWidth = (x2 - x1) * scaleX;
            const roiHeight = (y2 - y1) * scaleY;
            
            // Position ROI box correctly using coordinate calculations
            roiBox.style.display = 'block';
            roiBox.style.position = 'absolute';
            roiBox.style.left = `${roiLeft}px`;
            roiBox.style.top = `${roiTop}px`;
            roiBox.style.width = `${roiWidth}px`;
            roiBox.style.height = `${roiHeight}px`;
            roiBox.style.border = '3px solid var(--accent-primary)';
            roiBox.style.background = 'rgba(0, 255, 136, 0.2)';
            roiBox.style.zIndex = '15';
            
            // Debug parent containers - check key positioning properties
            ...
            if (roiOverlay) {
                const overlayStyles = window.getComputedStyle(roiOverlay);
                console.log('🔍 ROI Overlay key styles:', {
            ...
            if (previewContainer) {
                ...
                console.log('🔍 Preview Container key styles:', {
                ...
                // Fix the container positioning issue
            ...
            // Position ROI box correctly using coordinate calculations
            roiBox.style.display = 'block';
            roiBox.style.visibility = 'visible';
            roiBox.style.position = 'absolute';
            roiBox.style.left = `${roiLeft}px`;
            roiBox.style.top = `${roiTop}px`;
            roiBox.style.width = `${roiWidth}px`;
            ...
            // 🔍 Debug the ROI box element status
            console.log('🔍 ROI Box Element Status:');
            console.log('ROI Box:', roiBox);
            console.log('ROI Box parent:', roiBox.parentElement);
            console.log('ROI Box computed styles:', {
                display: window.getComputedStyle(roiBox).display,
                visibility: window.getComputedStyle(roiBox).visibility,
                opacity: window.getComputedStyle(roiBox).opacity,
                position: window.getComputedStyle(roiBox).position,
                zIndex: window.getComputedStyle(roiBox).zIndex,
                width: window.getComputedStyle(roiBox).width,
                height: window.getComputedStyle(roiBox).height
            });
            
            // Check parent container
            const overlay = document.querySelector('.roi-overlay');
            console.log('🔍 Overlay styles:', overlay ? {
            ...
            // ✅ Use the working red box approach but make it green
            ...
            console.log(`🎯 ROI Box Positioned: [${x1}, ${y1}, ${x2}, ${y2}] -> Box: ${roiLeft.toFixed(1)}, ${roiTop.toFixed(1)}, ${roiWidth.toFixed(1)}x${roiHeight.toFixed(1)}`);
        } else {
    ...
    handleCommandResponse(data) {
        ...
        this.addSystemMessage(`Command ${data.command_type}: ${data.message}`, level);
    }
    ...
    getPublicAPI() {
        return {
            ...
            handleCommandResponse: this.handleCommandResponse.bind(this),
            updateDailySummary: this.updateDailySummary.bind(this)
        };
    }
}