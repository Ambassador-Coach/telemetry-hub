...
export class TradeManager {
    ...
    handleOrderRejection(payload) {
        console.log('Order rejection:', payload);
        this.ui.addSystemMessage(`Order rejected: ${payload.reason || 'Unknown reason'}`, 'error');
    }

    // Handle order fill
    handleOrderFill(fill) {
        console.log('Order fill:', fill);
        this.ui.addSystemMessage(`Order filled: ${fill.symbol} ${fill.quantity} @ ${fill.price}`, 'success');
    }

    // Handle trade opened
    handleTradeOpened(trade) {
        console.log('Trade opened:', trade);
        this.ui.addSystemMessage(`Trade opened: ${trade.symbol} ${trade.action} ${trade.quantity}`, 'info');
    }

    // Handle trade closed
    handleTradeClosed(trade) {
        console.log('Trade closed:', trade);
        this.ui.addSystemMessage(`Trade closed: ${trade.symbol} P&L: ${trade.pnl}`, 'info');
    }
    ...
    getPublicAPI() {
        return {
            ...
            handleOrderRejection: this.handleOrderRejection.bind(this),
            handleOrderFill: this.handleOrderFill.bind(this),
            handleTradeOpened: this.handleTradeOpened.bind(this),
            handleTradeClosed: this.handleTradeClosed.bind(this),
